...
/**
 * Activity 作用域的共享 ViewModel，包装 [PhotoSessionHolder]，供 NavHost 和各 Screen
 * 通过 `hiltViewModel<NavSharedViewModel>()` 读取跨路由共享数据（选中行、Excel URI 等）。
 * Composable 不能直接 @Inject，需通过 hiltViewModel 间接访问 PhotoSessionHolder。
 */
@HiltViewModel
