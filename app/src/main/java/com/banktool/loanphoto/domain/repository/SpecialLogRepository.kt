 ...
 *
 * 与 [VisitNoteRepository] 同构，持久化用户在 AI 日报表页面输入的「补充说明/特殊日志」，
 * 按 excelUriMd5 区分。文件为 `special_logs/<md5>.txt`（纯文本，原子写），进入报告页时
 * 通过 [getSpecialLog] 回显，点击「保存」或「生成日报表」时通过 [saveSpecialLog] 持久化。
 */
