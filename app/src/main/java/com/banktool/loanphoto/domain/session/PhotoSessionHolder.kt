...
/**
 * 跨路由共享的拍照会话状态。CustomerRow 不是 Parcelable，无法通过
 * SavedStateHandle/路由参数传递，因此用 @Singleton 桥接
 * CustomerListScreen → Camera/Progress/Report。
 *
 * [excelUri]/[excelFileName]/[excelUriMd5] 由 CustomerListViewModel.loadExcel 设置，
 * [rows] 由 NavHost 的 onTakePhoto 回调设置（用户选中的行）。
 * 进程级单例：数据在 Activity 重建后保留，进程销毁后丢失（重新导入 Excel 即可恢复）。
 */
@Singleton
